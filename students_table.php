
	<form action="delete_stud.php" method="post">
	<table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
		<div class="pull-right">
	 <!--<a href="#" onclick="window.print()" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
	<a href="add_student.php" class="btn btn-inverse"><i class="icon-plus-sign icon-large"></i> Agregar alumno</a>-->
	</div>
	<!--<a data-toggle="modal" href="#student_delete" id="delete"  class="btn btn-danger" name=""><i class="icon-trash icon-large"></i> Delete</a>
-->
		<thead>
		<tr>
					<th>Full Name</th>
					<th>Gender</th>
					<th>Class</th>
					<th>Status</th>
					<th>Transport Route</th>
					<th class="empty"></th>
					<th class="empty"></th>
		</tr>
		</thead>
		<tbody>
        <!--
                <td class="empty" width="30"><input id="optionsCheckbox" class="uniform_on" name="selector[]" type="checkbox" value=""></td>
                <td class="empty" width="160">
                <a data-placement="left" title="Click to Edit" id="edit" href="edit_stud.php" class="btn btn-success"><i class="icon-pencil icon-large"></i> Edit</a>

                <a data-placement="top" title="Click to View all Details" id="view" href="view_stud.php" class="btn btn-warning"><i class="icon-search icon-large"></i> View</a>
        -->
		</td>
		</tr>

		</tbody>
	</table>
	</form>